...
<div>
    <ps-content-container class=" h-screen ">
        <!-- v-if="dataReady" -->
        <template #content>
            <div class="relative flex flex-col " style="height:93vh" > 
                
                <div class='lg:mt-32 mt-28 flex flex-col' >
                    <div class="flex items-center "> 
                        ...
                        <ps-route-link
                            ...
                            <ps-label textColor="text-secondary-400 dark:text-secondaryDark-grey "
                                class="hover:underline cursor-pointer font-medium text-xxs "> 
                                {{ $t("chat__account") }}                        </ps-label>      
                        </ps-route-link>
                        <ps-icon textColor="text-secondary-400 dark:text-secondaryDark-grey " name="rightArrow" class=' mx-0 sm:mx-2'/>
                        <div v-if="chatFlag == PsConst.CHAT_FROM_SELLER">
                            <ps-route-link
                            :to="{name: 'chat-list', query:{focus : 'sellerfocus'}}"> 
                                <ps-label textColor="text-secondary-400 dark:text-secondaryDark-grey " class="hidden sm:flex hover:underline cursor-pointer font-medium text-xxs "> {{ $t("chat__messages_from_seller") }}</ps-label>
                                <ps-label textColor="text-secondary-400 dark:text-secondaryDark-grey " class="flex sm:hidden hover:underline cursor-pointer font-medium text-xxs ">{{ $t("chat__messages") }}</ps-label>
                            </ps-route-link>
                            </div>
                            <div v-else>
                            <ps-route-link
                            :to="{name: 'chat-list' ,query:{focus : 'buyerfocus'}}"> 
                                <ps-label textColor="text-secondary-400 dark:text-secondaryDark-grey " class="hidden sm:flex hover:underline cursor-pointer font-medium text-xxs ">{{ $t("chat__messages_from_buyer") }}</ps-label>
                                <ps-label textColor="text-secondary-400 dark:text-secondaryDark-grey " class="flex sm:hidden hover:underline cursor-pointer font-medium text-xxs ">{{ $t("chat__messages") }}</ps-label>
                            </ps-route-link>
                        </div>
                        <ps-icon textColor="text-secondary-400 dark:text-secondaryDark-grey " name="rightArrow" class=' mx-0 sm:mx-2'/>
                        <div>
                            <ps-label textColor="text-secondary-400 dark:text-secondaryDark-grey " class="font-medium text-xxs "> {{ itemName }}</ps-label>
                        </div>
                    </div>

                    <div v-if="chatFlag == PsConst.CHAT_FROM_SELLER">
                        
                        <ps-label class="mt-1 text-2xl lg:text-3xl font-medium"> {{ $t("chat__messages_from_seller") }}</ps-label>
                    </div>
                    <div v-else>
                        <ps-label class="mt-1 text-2xl lg:text-3xl font-medium">{{ $t("chat__messages_from_buyer") }}</ps-label>
                    </div>
    
                    <div class="h-32 sm:h-20 lg:h-24 shadow-cardShadow">
                        <div class="flex flex-col bg-background dark:bg-backgroundDark w-full">
                            <div class="flex flex-row items-center w-full justify-end " >
                                <div class="flex-grow w-full justify-start flex flex-row">
                                    <div class=" lg:px-3 py-4 sm:px-2 px-1">
                                        <img alt="Placeholder" width="100px" height="100px" class="rounded-full bg-transparent lg:w-14 lg:h-14 w-11 h-11  flex object-cover" :src="userProvider.imageUrl(userProvider.user?.data ? userProvider.user.data?.userProfilePhoto : '',2)" @error="userProvider.defaultProfileImage" >
                                    </div>
                                    <div class="flex flex-col lg:ms-4 ms-2 my-auto">
                                        <ps-route-link :to="{
                                            name: 'other-profile',
                                            params: { user_id: userProvider.user?.data?.userId }}">
                                        <div class=" flex mt-0.5"> 
                                            <ps-label class="font-medium text-xs lg:text-base"> {{userProvider.user?.data?.userName }} </ps-label>
                                            <ps-icon v-if="userProvider.user.data && userProvider.user?.data?.isVerifybluemark == '1' " name="check" w="14" h="14" class="text-primary-500 dark:text-primaryDark-accent" />
                                        </div>
                                        </ps-route-link>
                                        
                                        <ps-route-link :to="{ name: 'item', params: { itemName: itemName.split(' ').join('-').toLowerCase() }, query: { item_id: itemId, item_name: itemName.split(' ').join('-').toLowerCase()   }}">                                        <div class=" flex flex-row"> 
                                            <ps-label class="font-medium text-xs lg:text-sm"> {{itemName}} </ps-label>
                                            <ps-label class="lg:ms-2 sm:ms-1 ms-0.5 font-light text-xxs lg:text-xs" textColor="text-primary-500 dark:text-primaryDark-accent"> {{condition}} </ps-label>                        
                                            
                                        </div>
                                        </ps-route-link>

                                        <div class=" flex flex-row"> 
                                            <ps-label class="font-medium text-xs lg:text-sm" textColor="text-primary-500 dark:text-primaryDark-accent">{{ currency }} {{ itemPrice }} </ps-label>
                                            <ps-label class="lg:ms-2 sm:ms-1 ms-0.5 font-light text-xxs lg:text-xs"> {{priceType}} </ps-label>
                                        </div> 
                                        
                                        <div v-if="isOtherUserOnline" class="my-1.5">
                                            <ps-label class=" font-light text-xxs" textColor="text-secondary-400 dark:text-secondaryDark-grey "> {{ $t("chat__online") }} </ps-label>
                                        </div>
                                        <div v-else class="m1-0.5">
                                            <ps-label class=" font-light text-xxs" textColor="text-secondary-400 dark:text-secondaryDark-grey "> {{ $t("chat__offline") }} </ps-label>
                                        </div>
                                ...
                                <div class="hidden sm:block w-48 pb-3">
                                    <div v-if="isSoldOut == '1' || chatHistoryProvider.chatHistory.data?.item.isSoldOut == '1'" class="flex flex-col content-end items-center">
                                        <div >
                                            <ps-label class="select-none px-5 py-2 my-auto bg-primary-500 dark:bg-primaryDark-accent text-median font-medium " 
                                            textColor="text-textLight "> {{ $t("chat__sold") }}</ps-label>
                                        </div>
                                    </div>
                                    <div v-else-if="chatFlag == PsConst.CHAT_FROM_SELLER && showOffer" class=" items-center">
                                        <div  @click = "makeOfferClicked">
                                            <ps-label class=" select-none px-5 py-2 my-auto bg-primary-500 dark:bg-primaryDark-accent text-sm font-medium  cursor-pointer" 
                                            textColor="text-textLight ">  {{ $t("chat__make_offer") }}</ps-label>
                                        </div>
                                    </div>
                                </div>
                                <div class="ms-2 hidden sm:flex lg:w-60 w-48 h-20 lg:h-24">
                                    <img alt="Placeholder"  width="400px" height="400px" class="w-48 h-20 lg:h-24 lg:w-60 object-cover" :src="chatHistoryProvider.imageUrl(itemImageName,2) " @error="chatHistoryProvider.defaultCarImage">
                                </div>
                            ...
                            <div class="sm:hidden flex-row flex justify-between">
                                <div class="flex flex-row ">
                                    <div class="w-44 pb-2">
                                        <div v-if="isSoldOut == '1' || chatHistoryProvider.chatHistory.data?.item.isSoldOut == '1'" class="flex flex-col content-end items-center">
                                            <div >
                                                <ps-label class="select-none px-5 py-2 my-auto bg-primary-500 dark:bg-primaryDark-accent text-xs text-median font-medium " 
                                                textColor="text-textLight"> {{ $t("chat__sold") }}</ps-label>
                                            </div>
                                        </div>
                                        <div v-else-if="chatFlag == PsConst.CHAT_FROM_SELLER && showOffer" class="flex flex-col content-end items-center">
                                            <div  @click = "makeOfferClicked">
                                                <ps-label class="select-none px-5 py-2 my-auto bg-primary-500 dark:bg-primaryDark-accent text-xs font-medium  cursor-pointer" 
                                                textColor="text-textLight "> {{ $t("chat__make_offer") }}</ps-label>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class=" flex justify-end items-end mt-1">
                                    <div class="h-11 w-16 me-1 mb-1">
                                        <img alt="Placeholder"  width="400px" height="400px" class="w-16 h-11 object-cover" :src="chatHistoryProvider.imageUrl(itemImageName,2) " @error="chatHistoryProvider.defaultCarImage" >
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div ref="messages" v-on:scroll="handleScroll" id="messages"  :class="isScrolling ? '' : 'no-scrollbar'"
                    class="px-4 flex flex-col-reverse bg-primary-000 dark:bg-primaryDark-black  overflow-y-auto h-full scrolling-touch  ">
                        ...
                        <div class=" " v-if='chat.message != "" && chat.message != null '>
                            ...
                            <div v-else-if="chat.type == PsConst.CHAT_TYPE_TEXT">
                                    ...
                                    <div v-else>
                                        ...
                                        <div v-if='chat.sendByUserId == loginUserId' class="w-full ps-1/3 flex flex-col justify-end items-end">
                                            ...
                                            <div v-if="chat.offerStatus == PsConst.CHAT_STATUS_ACCEPT && chat.isUserBought == false" class="me-2">
                                                <ps-label @click="isUserBought(chat)" class="p-2 flex flex-shrink-0 bg-primary-500 dark:bg-primaryDark-accent rounded-lg cursor-pointer " textColor="text-textLight "> {{ $t("chat__is_user_bought") }} </ps-label>
                                            </div>
                                        </div>
                                ...
                                <div v-else>
                                    ...
                                    <div v-if='chat.sendByUserId == loginUserId' class='w-full flex justify-end ps-1/3 flex-row items-end'>
                                        <ps-label-caption-2 class="me-2 mb-2"> {{chat.timeString}} </ps-label-caption-2>
                                        <span class="px-4 py-2 mt-2 mb-1 rounded-lg inline-block rounded-br-none bg-primary-000 dark:bg-secondaryDark-black  ">
                                            <ps-label> {{chat.message}} </ps-label> 
                                        </span>
                                    </div> 
                                    ...
                                    <div v-else class="pe-1/3 flex flex-row items-end">
                                        <span class="px-4 py-2 mt-2 mb-1 rounded-lg inline-block rounded-bl-none bg-primary-000 dark:bg-primaryDark-black ">
                                            <ps-label> {{chat.message}} </ps-label> 
                                        </span>
                                        <ps-label-caption-2 class="ms-2 mb-2"> {{chat.timeString}} </ps-label-caption-2>
                                    </div>      
                                ...
                                <div  class="w-full flex flex-col justify-center items-center">
                                    ...
                                    <div >
                                        <div @click="clickGiveReview()">
                                            <ps-label class="p-2 mb-1 flex flex-shrink-0 bg-primary-500 dark:bg-primaryDark-accent cursor-pointer rounded-lg " textColor="text-textLight "> {{ $t("chat__give_review") }} </ps-label>
                                        </div>
                                    </div>
                                    <div v-if='chat.sendByUserId == loginUserId && chat.offerStatus == PsConst.CHAT_STATUS_IS_USER_BOUGHT'>
                                        <ps-label @click="markAsSold(chat)" class="p-2 flex flex-shrink-0 bg-primary-500 dark:bg-primaryDark-accent cursor-pointer rounded-lg" textColor="text-textLight "> {{ $t("chat__mark_as_sold") }} </ps-label>
                                    </div>
                                </div>
                            ...
                            <div v-else-if="chat.type == PsConst.CHAT_TYPE_SOLD">
                                ...
                                <div  class="w-full flex flex-col justify-center items-center">
                                    ...
                                    <div  class='flex  flex-row '>
                                        <span class="px-4 py-2 mt-2 mb-1 rounded-xl lg:rounded-2xl inline-block bg-primary-000 dark:bg-primaryDark-black">
                ...
                <!-- Send Message -->
                <div class=" absolute inset-y-full w-full">
                    <div class="relative flex shadow-barShadow">
                        ...
                        <ps-input v-bind:placeholder="$t('chat__write_message')" @keyup.enter="sendNormalMessage(inputMessageController)"  v-model:value="inputMessageController" class="w-full ps-12 py-3 me-8 " 
                        theme="form-input outline-none bg-primary-000 dark:bg-primaryDark-black text-secondary-500 dark:text-secondaryDark-white"/>
                        <div class="absolute right-0 items-center mb-0 ">
                            <button type="button" @click="sendNormalMessage(inputMessageController)" class="inline-flex items-center justify-center h-11 w-16 transition duration-500 ease-in-out text-textLight bg-primary-500 dark:bg-primaryDark-accent  focus:outline-none">
...
export default {
    ...
    setup() { 
        ...
        const priceType = route.query.priceType?.toString() ?? '';
        itemPrice = route.query.item_price?.toString() ?? '';
        ...
        async function sendMessage(
            ...
            if(chatHolder.offerStatus == PsConst.CHAT_STATUS_NULL){
        ...
        async function isUserBought( chat){
            ...
            userBoughtHolder.isUserOnline = "0"; 
            if(isOtherUserOnline.value) {
