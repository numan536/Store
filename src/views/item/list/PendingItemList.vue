        ...
        <template #content>
            <div class="sm:mt-32 lg:mt-36 mt-28 mb-16 ">
                <div class=" flex items-center "> 
                        ...
                        <ps-label textColor="text-secondary-400 dark:text-secondaryDark-grey " class="font-light text-xxs " > {{ $t("pending_item_list__pending_item_list") }}  </ps-label>
                    </div>

                <div class="flex flex-col items-start">
                    <ps-label-header-4 class=" mb-1 font-medium"> {{ $t("pending_item_list__pending_item_list") }} </ps-label-header-4>
                    <ps-label class="text-xs lg:text-sm  mb-5 font-light">{{ $t("pending_item_list__text") }}</ps-label>
                        ...
                        <div class="w-full">
                            ...
                            <div v-if="itempendingProvider.productList?.data != null">
                                <div class="grid lg:grid-cols-4 sm:grid-cols-3 grid-cols-1  gap-4 sm:gap-3.5 lg:gap-4">
                                    ...
                                    <div class="w-full " v-for="item in itempendingProvider.productList.data" :key="item.id">
...
export default {
    ...
    setup() {
        ...
        async function refleshData(){
            ...
            await itempendingProvider.refleshItemList(loginUserId, holder);
