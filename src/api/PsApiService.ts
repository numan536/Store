...
export default class PsApiService extends PsApi {
    ...
    **/
    ...
    /**
         ...
         * @param offset 
         */
    static async getManufacturerList<T extends PsObject<T>>(obj: T, limit: Number, offset: Number, loginUserId: String, postData: any): Promise<PsResource<T[]>> {
       
        const url: String = PsUrl.ps_manufacturer_url + "/limit/" + limit + "/offset/" + offset + "/login_user_id/" + loginUserId;
        return await PsApi.postData<T, T[]>(obj, url, postData);
    ...
    static async searchModelList<T extends PsObject<T>>(obj: T, loginUserId: String, limit: Number, offset: Number, postData: any): Promise<PsResource<T[]>> {

        const url: String = PsUrl.ps_model_search_url + "/login_user_id/" + loginUserId + "/limit/" + limit + "/offset/" + offset;
        return await PsApi.postData<T, T[]>(obj, url, postData);
    }


    static async postModelScribe<T extends PsObject<T>>(obj: T, postData: any): Promise<PsResource<T[]>> {
        console.log(postData);
        const url: String = PsUrl.ps_model_subscription_url;
        return await PsApi.postData<T, T[]>(obj, url, postData);

    }

    static async getAllModelList<T extends PsObject<T>>(obj: T, manufacturerId: String,limit: Number, offset: Number): Promise<PsResource<T[]>> {
        const url: String = PsUrl.ps_model_url + "/manufacturer_id/" + manufacturerId + "/limit/" + limit + "/offset/" + offset;
        return await PsApi.getData<T, T[]>(obj, url);
    }
    ...
    /// Item Entry
    ///
    static async postItemEntry<T extends PsObject<T>>(obj: T,loginUserId: string, postData: any): Promise<PsResource<T>> {
    ...
    static imageUrl = (path: String, size: number = 0) => {
        ...
        if(appInfoProvider.isThumbnailGenerate == true){
            if(size == 1) {
