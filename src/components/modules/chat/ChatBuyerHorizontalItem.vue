<template>
     <div class="cursor-pointer w-full" v-on:click="onClick != null ? onClick(chathistory) : null">
        ...
        <!-- Pscard -->
        <ps-card class="h-32 sm:h-20 lg:h-24 hover:shadow-cardShadow" :enabledHover="false">
            ...
            <div class="flex flex-row items-center w-full justify-end " >
                <div class="flex-grow w-full justify-start flex flex-row">
                    <div class="lg:w-14 lg:h-14 w-11 h-11 lg:mx-3 sm:mx-2 my-2 mx-1">
                        <img alt="Placeholder" width="100px" height="100px" class="rounded-full bg-transparent lg:w-14 lg:h-14 w-11 h-11  flex object-cover" :src="chathistorylistProvider.imageUrl(chathistory ? chathistory.buyer.userProfilePhoto : '',2)" @error="chathistorylistProvider.defaultProfileImage" >
                    </div>
                    <div class="flex flex-col lg:ms-2 ms-1 my-auto">
                        <div class=" flex mt-0.5"> 
                            <ps-label class="font-medium text-xs lg:text-base"> {{chathistory ? chathistory.buyer.userName : ''}} </ps-label>
                        ...
                        <div class=" flex flex-row"> 
                            ...
                            <!-- <ps-label class="font-medium text-xs lg:text-sm" textColor="text-primary-500 dark:text-primaryDark-accent"> {{chathistory ? chathistory.item.itemCurrency.currencySymbol : ''}} {{ formatPrice(chathistory ? chathistory.item.price : '')}}  </ps-label> -->
                            <ps-label class="lg:ms-2 sm:ms-1 ms-0.5 py-1.5 font-light text-xxs lg:text-xs"> {{chathistory ? chathistory.item.itemPriceType.name : ''}} </ps-label>
                        </div>
                        <div class="">
                            <ps-label class=" font-light text-xxs" textColor="text-secondary-400 dark:text-secondaryDark-grey "> {{chathistory ? chathistory.addedDateStr : ''}} </ps-label>
                        </div>

                    </div>
                </div>
                
                <div class="hidden sm:flex flex-col w-36 lg:w-40 ">
                    <div class="flex flex-col items-center justify-center mt-3">
                    <div class="flex flex-col">
                        <ps-label class="text-xs lg:text-sm font-medium text-center flex-shink-0 mx-auto" textColor="text-primary-500 dark:text-primaryDark-accent" > {{chathistory ? chathistory.sellerUnreadCount : '0'}}  </ps-label>
                        <div v-if="chathistory?.sellerUnreadCount != '1'">
                            <ps-label class="font-light text-center text-3xs sm:text-xxs" textColor="text-primary-500 dark:text-primaryDark-accent"  > {{ $t('chat_horizontal_item__message') }} </ps-label>
                        </div>
                        <div v-else>
                            <ps-label class="font-light text-center text-3xs sm:text-xxs" textColor="text-primary-500 dark:text-primaryDark-accent" > {{ $t('chat_horizontal_item__messages') }} </ps-label>
                        </div>
                    </div>
                    </div>
                </div>
                <div class="hidden sm:flex lg:ms-12 sm:ms-6 lg:w-60 w-48 h-20 lg:h-24">
                    <img alt="Placeholder"  width="400px" height="400px" class="w-48 h-20 lg:h-24 lg:w-60 object-cover" :src="chathistorylistProvider.imageUrl(chathistory ? chathistory.item.defaultPhoto.imgPath : '',3) " @error="chathistorylistProvider.defaultCarImage" >
                </div>
            ...
            <div class="sm:hidden flex-row flex justify-between">
                <div class="flex flex-row ">
                    <div class="flex flex-row mt-6">
                        <ps-label class="text-xs lg:text-sm font-medium text-center mx-2" textColor="text-primary-500" > {{chathistory ? chathistory.sellerUnreadCount : '0'}}  </ps-label>
                        <div v-if="chathistory?.sellerUnreadCount != '1'" class="mt-1">
                            <ps-label class="font-light text-center text-3xs"  > {{ $t('chat_horizontal_item__message') }} </ps-label>
                        </div>
                        <div v-else>
                            <ps-label class="font-light text-center text-3xs"  > {{ $t('chat_horizontal_item__messages') }} </ps-label>
                        </div>
                    </div>
                </div>
                <div class=" flex justify-end items-end mt-1">
                    <div class="h-11 w-16 me-1 mb-1">
                        <img alt="Placeholder"  width="400px" height="400px" class="w-16 h-11 object-cover" :src="chathistorylistProvider.imageUrl(chathistory ? chathistory.item.defaultPhoto.imgPath : '',3) " @error="chathistorylistProvider.defaultCarImage" >
                    </div>
