<template>
    ...
    <div v-if="parseInt(gallery.ordering) <= imageCount" class='w-screen bg-primary-000 dark:bg-primaryDark-black overflow-hidden h-56 flex flex-row justify-center items-center'  >         
        <div v-if="gallery.imgType == 'video-icon' " class=' relative w-full h-full' >
            
            <div class="absolute w-full h-56">
                <img id="slider-full" alt="Placeholder" width="400px" height="400px" class="h-56 w-full " 
        ...
        <div v-else class='  w-full h-full ' >
            ...
            <img id="slider-full" alt="Placeholder" width="400px" height="400px" class="w-full h-56" 
                :class="customClass " label="Image 1" 
                v-if="gallery != null && gallery.imgPath != null" 
                :src="galleryProvider.imageUrl(gallery ? gallery.imgPath : '',3)"  
                @error="galleryProvider.defaultCarImage" >
