...
<div>
    <div class="cursor-pointer h-full w-full" v-on:click="onClick != null ? onClick(user) : null">
        <!-- admin Pscard -->
        <ps-card v-if="user" class="flex-col h-full bg-primary-000 dark:bg-primaryDark-black">
            <div class="p-4 bg-primary-000 dark:bg-primaryDark-black rounded-xl lg:rounded-2xl">
                <div class="flex items-start justify-between leading-none" >
                    <div class="flex items-start no-underline cursor-pointer justify-between">
                        <ps-route-link  :to="{ name: 'other-profile', params: {user_id: user.userId } }">
                        <div class="flex flex-row">
                            <div class="flex mt-2 w-10 h-10">
                            ...
                            <div class="flex flex-col ms-2 w-36 sm:w-32">
                                <div class="flex">
                                    <ps-label class="font-medium lg:text-base text-sm truncate w-32 sm:w-28" > {{ user ? user.userName : '' }} </ps-label>
                            ...
                            <ps-label v-if="user && user.isShowPhone == '1' " class="font-light text-xxs lg:text-xs  mt-0.5"> {{  user ? user.userPhone :''  }} </ps-label>
                                <ps-label class="font-light text-xxs lg:text-xs  mt-0.5"> {{  user.userAboutMe.length > 20 ? user.userAboutMe.slice(0,20)+' ....' : user.userAboutMe  }} </ps-label>
                            </div>
                        </div>
                        </ps-route-link>
                        <div v-if="user.userId != LoginUserId" @click="followClick(user)">
                            <div class="flex ms-5 mt-2" v-if="user.isFollowed == '0'">
                                <ps-button rounded="rounded-2xl" textSize="text-xxs lg:text-xs" > {{ $t("profile__follow") }} </ps-button>
                            </div>
                            <div class="flex ms-5 mt-2" v-else>
                                <ps-button rounded="rounded-2xl" textSize="text-xxs lg:text-xs" > {{ $t("profile__unfollow") }} </ps-button>
                            </div>
                        </div>
