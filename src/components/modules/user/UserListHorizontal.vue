<template>
    <div class="cursor-pointer h-full w-full" v-on:click="onClick != null ? onClick(user) : null">
        <!-- Pscard -->
         <ps-card class="flex-col h-full bg-secondary-000 dark:bg-primaryDark-black lg:p-5 p-3">
            <div class="w-full">
            ...
            <div  class="w-full">
                ...
                <div class="lg:mt-2 mt-1.5 flex justify-between">
                    <ps-label class="font-light text-sm">{{ $t("user_list_horizontal__followers") }} </ps-label>
                    <ps-label class="font-medium text-xl"> {{user ? user.followerCount:'0'}}</ps-label>
                </div>
                <div class="lg:mt-2 mt-1.5 flex justify-between">
                    <ps-label class="font-light text-sm">{{ $t("user_list_horizontal__followings") }} </ps-label>
                    <ps-label  class="font-medium text-xl">{{user ? user.followingCount:'0'}}</ps-label>
                </div>
