            ...
            <div class="px-2 py-2 bg-background dark:bg-primaryDark-black" :class="psValueHolder.showProfile == 'show' ? ' pt-0' : ''">
                <div class="relative h-36 ">
                    ...
                    <div v-else class=" relative h-28 flex justify-between">  
                         ...
                         <div v-if="loginUserId != item?.user?.userId" class="flex flex-col mt-2 me-2">
                            <ps-label  @click="favouriteClicked">
                                <div>
                                    <ps-icon name="favourite" w="24" h="24" class="cursor-pointer text-primary-500 dark:text-primaryDark-accent " v-if="item?.isFavourited == '1'"/>
                                    <font-awesome-icon :icon="['far', 'heart']" class="cursor-pointer text-primary-500 dark:text-primaryDark-accent text-xl" v-else  />
                                </div>
                            </ps-label>
                             <ps-route-link class="flex-grow cursor-pointer " :to="{ name: 'item',params: { itemName: item.title.split(' ').join('-').toLowerCase() }, query: { item_id: item?.id, item_name: item?.title.split(' ').join('-').toLowerCase()  }}">
                            </ps-route-link>
