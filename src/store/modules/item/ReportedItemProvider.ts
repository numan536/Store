...
export class ReportedItemProvider extends PsProvider {
    ...
    async refleshItemList(loginUserId: string) {
        this.loading.value = true;
        const tempOffset = 0;
        const tempLimit = this.reportedReportedItemList.data.length;
        
        const responseData = await PsApiService.getReportedItemList<Product>(new Product(), loginUserId, tempLimit, tempOffset);
        this.reportedReportedItemList = responseData;
