...
export class ProductProvider extends PsProvider {
    ...
    async refleshItemList(loginUserId: string, holder: ProductParameterHolder) {
        this.loading.value = true;
        const tempOffset = 0;
        const tempLimit = this.productList.data.length;
    ...
    async loadPaidProductList(loginUserId:string, holder: ProductParameterHolder) {
    ...
    async refleshPaidProductList(loginUserId:string, holder: ProductParameterHolder) {
    ...
    async submitItemEntry(loginUserId : string, holder: ItemEntryParameterHolder): Promise<PsResource<Product>> {
        this.loading.value = true;
        this.product = await PsApiService.postItemEntry<Product>(new Product(),loginUserId, holder.toMap());
        this.loading.value = false;
